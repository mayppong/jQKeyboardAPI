...
jQuery(document).on("keystroke", function(event, keyData) {
	console.log(keyData);
});
...
test( "Hit Letter a once", function() {
	...
	equal( keyData.character, "a", 	"Corrent character code converted" );
...
test( "Hit CTRL+V", function() {
	...
	equal( keyData.keyType, "MODIFIED_KEY", "Correct key type assigned" );
